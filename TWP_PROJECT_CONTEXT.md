...
- **SHIPPED: You-page data synced separately from workouts.** `buildSyncPayload()` nests `userName`, `profile`, `measurementHistory`, `weeklyGoal` under a `youData` key, distinct from `workouts`. `pullSync()` merges `youData` independently: `userName`/`weeklyGoal` only fill in if unset locally (local wins on conflict), `profile` fills gaps without overwriting locally-set values, `measurementHistory` unions per-field by date. Fixes a bug where You-page fields (name, photo, measurements, goal) weren't persisting through sync. Rationale: workout deletes/imports and You-page edits are independent actions, so neither payload should clobber the other.
...
- `state.agent = { memory, unlockedConcepts[], interactionHistory[], lastAnalyzedWorkoutId, pendingTip, pendingTipPinned }`, persisted via the normal `loadState`/`saveState` path (defensive-merged on load so pre-existing saved state without `agent` doesn't break). **Not** currently included in the cloud-sync payload (`buildSyncPayload`) — kept local-only so chat history doesn't bloat every GitHub-committed sync.
...
## SHIPPED: Coach tip pin/dismiss + typing indicator
- `state.agent.pendingTipPinned` (new field, defaults `false`) — when `true`, `evolveAndAnalyze()` and `checkInIfIdle()` skip overwriting `pendingTip` with a fresh one, so a tip the athlete wants to keep visible survives the next post-workout/idle coaching cycle. `checkInOnInjury()` is **not** gated by the pin (injury-driven tips always take priority for safety).
- Tip card (Coach tab, both the maximized-chat and normal layouts) now renders from one shared `tipCardHTML` template string (previously duplicated inline in both branches of `renderCoachScreen()`) with two new controls next to "🧠 View memory": `📌 Pin`/`📌 Pinned` (`AICoach.pinTip()` toggles the flag, re-renders, toasts) and `✕ Dismiss` (`AICoach.dismissTip()` clears `pendingTip`, unpins, marks seen, updates the nav badge). Both controls only render when a real tip is present (`hasTip`).
- Chat loading state: the static "Coach is thinking…" text bubble replaced with an animated 3-dot typing indicator (`.typing-dot` spans + `typingBounce` CSS keyframe, staggered `animation-delay`), matching the same message-bubble container so it slots into the existing chat flow.
